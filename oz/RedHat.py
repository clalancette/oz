...
class RedHatCDYumGuest(RedHatCDGuest):
    ...
    def _customize_repos(self, guestaddr):
        ...
        for repo in self.tdl.repositories.values():
            ...
            def create_certfiles(repo, cert, fileext, propname):
                certdict[propname] = {}
            ...
            try:
                if repo.clientcert:
                    create_certfiles(repo, repo.clientcert, "client.crt",
                                     "sslclientcert")
                if repo.clientkey:
                    create_certfiles(repo, repo.clientkey, "client.key",
                                     "sslclientkey")
                if repo.cacert:
                    create_certfiles(repo, repo.cacert, "ca.pem", "sslcacert")
                ...
                # TODO: Clean these up on the remote end of things if we fail anywhere below 
