    ...
    def _check_pvd(self, iso):
        ...
        if self.tdl.distro == "RHEL-4":
            if self.tdl.installtype == 'iso':
                ...
                if not re.match("RHEL/4(-U[0-9])?", iso.pvd.volume_identifier) or (iso.pvd.space_size * 2048) < 1 * 1024 * 1024 * 1024:
            ...
            if self.tdl.installtype == 'iso':
                if not re.match(r"CentOS 4(\.[0-9])?.*DVD", iso.pvd.volume_identifier):
                    raise oz.OzException.OzException("Only DVDs are supported for CentOS-4 ISO installs")
            else:
                # url installs
                if not re.match("CentOS *", iso.pvd.volume_identifier):
